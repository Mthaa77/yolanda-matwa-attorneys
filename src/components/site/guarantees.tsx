...
const commitments = [
  [FileSignature, "A written quote before work begins", "You see the professional fee, statutory costs and disbursements in writing before you commit."],
  [Phone, "A named contact for your matter", "Your matter is never reduced to a reference number. You know who is handling it and where to turn for clarity."],
  [Clock, "Updates at meaningful milestones", "From lodgement to registration, you receive clear communication at the points that matter."],
  [Lock, "Your information handled with care", "Your private legal information is treated with discretion and responsible process management."],
  [RefreshCw, "Scope changes discussed early", "When a matter changes, the conversation happens before the cost changes."],
  [ShieldCheck, "Credentials that inspire confidence", "A practice shaped by admitted legal expertise, notarial and conveyancing capability."],
] as const;
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[linear-gradient(180deg,#fdfbf7_0%,#eef1f6_100%)] py-24 sm:py-32">
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
        <div className="overflow-hidden rounded-[2rem] border border-navy/10 bg-navy-deep px-6 py-10 text-cream shadow-[0_32px_80px_-46px_rgba(15,31,56,0.8)] sm:px-9 sm:py-12 lg:px-12 lg:py-14">
          ...
          <div className="relative grid gap-10 lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] lg:gap-14">
            <div className="flex flex-col justify-between">
              ...
              <div className="mt-10 rounded-[1.4rem] border border-gold/25 bg-[linear-gradient(145deg,rgba(176,141,69,0.18),rgba(255,255,255,0.05))] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.12)]">
                ...
                <a href="#contact" className="group mt-6 inline-flex items-center gap-2 rounded-full bg-gold px-5 py-3 text-sm font-bold text-navy-deep transition-all duration-300 hover:-translate-y-0.5 hover:bg-gold-light">
            ...
            <StaggerGroup className="grid gap-4 sm:grid-cols-2 lg:content-start">
              {commitments.map(([Icon, title, description], index) => (
                ...
                >
                  ...
                  <span className="pointer-events-none absolute -right-3 -top-8 font-display text-[6.8rem] font-bold leading-none text-white/[0.035]">{String(index + 1).padStart(2, "0")}</span>
                  <div className="relative">
                    <span className="flex h-11 w-11 items-center justify-center rounded-2xl border border-gold/30 bg-gold/10 text-gold-light transition-transform duration-500 group-hover:scale-110 group-hover:[transform:translateZ(14px)]">
                      <Icon className="h-5 w-5" />
                    </span>
                    <h3 className="mt-5 font-display text-lg font-bold leading-snug text-cream">{title}</h3>
                    <p className="mt-2 text-sm leading-relaxed text-cream/64">{description}</p>
                  </div>
