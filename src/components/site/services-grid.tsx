...
export function ServicesGrid() {
  ...
  return (
    <section
      id="services"
      className="relative isolate overflow-hidden bg-mist py-24 sm:py-32"
    >
      {/* Cinematic backdrop — deliberately subtle so the firm stays the focal point. */}
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
        <motion.div
          ...
          className="relative overflow-hidden rounded-[2rem] border border-white/15 bg-navy-deep px-6 py-8 shadow-navy-deep sm:px-10 sm:py-11 lg:px-14"
        >
          ...
          <div className="relative grid gap-9 lg:grid-cols-[1fr_auto] lg:items-end">
            <div className="max-w-3xl">
              ...
              <h2 className="mt-6 max-w-2xl font-display text-4xl font-bold leading-[1.04] text-cream sm:text-5xl lg:text-6xl">
                Counsel that feels as considered as your next move.
              </h2>
              <p className="mt-5 max-w-2xl text-base leading-relaxed text-cream/72 sm:text-lg">
                Clear direction, meticulous legal work, and personal attention across the moments that matter most.
              </p>
            </div>

            <div className="flex items-center gap-4 lg:justify-self-end">
              <div className="relative shrink-0">
                <div className="absolute -inset-2 rounded-full bg-gold/25 blur-xl" />
                <div className="relative overflow-hidden rounded-full border border-gold/40 bg-cream p-1 shadow-[0_10px_28px_-12px_rgba(0,0,0,0.8)]">
                  <Image
                    src="/images/logo.png"
              ...
              <div className="hidden border-l border-white/15 pl-4 sm:block">
                <p className="text-[0.63rem] font-bold uppercase tracking-[0.2em] text-gold-light/90">
                  Yolanda Matwa Attorneys
                </p>
                <p className="mt-1 text-sm text-cream/60">For all your legal needs</p>
              </div>
            </div>
          </div>
        </motion.div>

        <div className="relative mt-8 rounded-[2rem] border border-white/80 bg-white/70 p-3 shadow-premium backdrop-blur-sm sm:p-4">
            ...
            {SERVICES.map((service, index) => {
              ...
              return (
                <motion.button
                  ...
                  className={cn(
                    "group relative min-h-[22rem] overflow-hidden rounded-[1.55rem] border border-border/80 bg-white p-6 text-left shadow-[0_8px_30px_-24px_rgba(15,31,56,0.55)] transition-[border-color,box-shadow] duration-500 hover:border-gold/45 hover:shadow-[0_20px_52px_-24px_rgba(15,31,56,0.38)] sm:p-7 [perspective:1200px]",
                    service.featured &&
                      "bg-[linear-gradient(135deg,#FFFFFF_0%,#FCFAF5_60%,#F2E9D5_100%)] md:col-span-2 lg:col-span-2",
                  )}
                >
                  {/* Gold reflection, depth planes, and engraved service sequence. */}
                  ...
                  <div className="relative flex h-full flex-col [transform-style:preserve-3d]">
                    <div className="flex items-start justify-between gap-5">
                      <span
                        className={cn(
                          "relative flex h-[4.35rem] w-[4.35rem] shrink-0 items-center justify-center rounded-2xl border transition-transform duration-500 group-hover:scale-110 group-hover:[transform:translateZ(22px)]",
                          accent.icon,
                        )}
                      >
                        <span className="absolute inset-1 rounded-[0.8rem] border border-white/50" />
                        <Icon className="relative h-7 w-7" strokeWidth={1.7} />
                      </span>
                    ...
                    <h3
                      className={cn(
                        "mt-7 max-w-xl font-display font-bold leading-[1.08] text-navy-deep transition-transform duration-500 group-hover:[transform:translateZ(18px)]",
                      ...
                      <div className="mt-6 grid max-w-2xl grid-cols-1 gap-x-6 gap-y-2 border-t border-gold/20 pt-5 sm:grid-cols-2">
                        {service.covers.slice(0, 4).map((cover) => (
                          <span key={cover} className="flex items-center gap-2 text-sm text-navy/75">
                            <span className="flex h-4 w-4 items-center justify-center rounded-full bg-gold/12 text-gold">
          ...
          <div className="relative mx-2 mt-3 flex flex-col gap-4 rounded-[1.35rem] border border-navy/10 bg-[linear-gradient(100deg,rgba(238,241,246,0.92),rgba(255,255,255,0.92))] px-5 py-5 sm:mx-0 sm:flex-row sm:items-center sm:justify-between sm:px-7">
            <div>
